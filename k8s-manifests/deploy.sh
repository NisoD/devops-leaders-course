...
echo "1. Get the NodePort with: kubectl get service devops-leaders-app-service -n ${NAMESPACE}"
echo "   (Look for the PORT(S) column, e.g. 80:31234/TCP; 31234 is the NodePort)"
echo "2. Get the kind node IP with: kubectl get nodes -o wide"
echo "   (Use the INTERNAL-IP, usually 'localhost' for kind)"
echo "3. Open your browser to: http://localhost:<nodeport> (replace <nodeport> with the value from step 1)" 